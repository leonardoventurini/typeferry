"""Per-connection state — port of ``bifrost-ts/src/server/client-node.ts``.
...
class ClientNode:
    ...
    def __init__(
        self,
        server: Server,
        socket: BifrostSocket | None = None,
...
async def _send(socket: BifrostSocket, payload: str) -> None:
