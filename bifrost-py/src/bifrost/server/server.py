"""Top-level :class:`Server` — port of ``bifrost-ts/src/server/server.ts``.
...
from bifrost.protocol.constants import NO_CHANNEL, BifrostEvents, ServerEvents
from bifrost.server.client_node import ClientNode
from bifrost.server.default_methods import build_default_methods
from bifrost.server.event import Event, EventOptions
from bifrost.server.method import Method, MethodFunction, MethodOptions
...
ServerEventHandler = Callable[[Any], None | Awaitable[None]]
...
class Server(ServerChannel):
    """Transport-agnostic Bifrost server core.
    ...
    def __init__(self, options: ServerOptions | None = None) -> None:
        ...
        self.add_event(BifrostEvents.METHOD_REFRESH.value)
    ...
    def set_auth(self, setup: AuthSetup) -> None:
        ...
        from bifrost.protocol.constants import Methods
...
# Re-export for ``from bifrost.server import EventOptions``
__all__ = [
    "AuthSetup",
    "EventOptions",
    "Server",
