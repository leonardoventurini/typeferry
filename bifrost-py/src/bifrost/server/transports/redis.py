...
``bifrost-ts/src/server/transports/redis-transport.ts``.

Propagates events across Bifrost server instances via Redis pub/sub.
See PROTOCOL.md §2.3.
...
* Payload: EJSON-encoded ``{event, channel, message, excludeUuid?}``
  ...
  :meth:`bifrost.server.event.Event.handler`.

Keys maintained:

* ``bifrost:servers`` — SET of active server UUIDs
* ``bifrost:clients:<server-uuid>`` — SET of clients per server
* ``bifrost:users:<server-uuid>`` — SET of active users per server
"""
...
from bifrost.protocol.constants import NO_CHANNEL, RedisListeners, ServerEvents
...
class RedisTransport:
    ...
    async def close(self) -> None:
        ...
        if self._pub is not None:
            with contextlib.suppress(Exception):
                await cast(Any, self._pub.delete(f"bifrost:clients:{self.server.uuid}"))
                await cast(
                    Any, self._pub.srem("bifrost:servers", self.server.uuid)
                )
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        servers = await cast(Any, self._pub.smembers("bifrost:servers"))
        for server_uuid in servers:
            client_count += int(
                await cast(Any, self._pub.scard(f"bifrost:clients:{server_uuid}"))
            )
            user_count += int(
                await cast(Any, self._pub.scard(f"bifrost:users:{server_uuid}"))
            )
            members = await cast(
                Any, self._pub.smembers(f"bifrost:users:{server_uuid}")
            )
    ...
    async def _listen(self, pubsub: Any) -> None:
        async for raw in pubsub.listen():
            ...
            try:
                ...
                _log.exception("Malformed Bifrost Redis message")
                continue
