...
``bifrost-ts/src/server/transports/websocket-transport.ts``.

Implements PROTOCOL.md §2.2 on top of Starlette's ``WebSocketRoute``:

* ``/bifrost-ws`` upgrade with query params ``uuid``/``token``/``meta``
* origin validation
...
* RPC / RPC-void dispatch via :mod:`bifrost.server.transports.ws_shared`
...
from bifrost.protocol.constants import BIFROST_WS_PATH, ServerEvents
from bifrost.server.client_node import ClientNode
from bifrost.server.room_registry import RoomRegistry
from bifrost.server.socket import SocketState
...
class StarletteSocket:
    ...
    :class:`BifrostSocket` interface (``readyState``/``send``/``close``
    + stable identity).
    """
...
class WebSocketTransport:
    ...
    async def _endpoint(self, ws: WebSocket) -> None:
        ...
        try:
            ...
            _log.exception("Bifrost WS connection raised")
