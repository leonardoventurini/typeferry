...
from bifrost.protocol.constants import NO_CHANNEL, BifrostEvents, Methods
...
def test_method_refresh_event_auto_registered() -> None:
    server = _server()
    assert BifrostEvents.METHOD_REFRESH.value in server.events
...
async def test_rpc_on_off_subscribe_and_unsubscribe() -> None:
    ...
    class Socket:
        from bifrost.server.socket import SocketState as _S
    ...
    assert server.websocket_transport.rooms.has(
        node.socket, "bifrost:room-1:chat:message"
    )
