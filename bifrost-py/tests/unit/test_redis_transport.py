...
from bifrost.protocol.constants import NO_CHANNEL, RedisListeners
from bifrost.server.server import Server, ServerOptions
from bifrost.server.transports.redis import RedisTransport
...
async def test_get_stats_aggregates_across_servers() -> None:
    ...
    fake.sets["bifrost:servers"] = {"srv-a", "srv-b"}
    fake.sets["bifrost:clients:srv-a"] = {"c1", "c2"}
    fake.sets["bifrost:clients:srv-b"] = {"c3"}
    fake.sets["bifrost:users:srv-a"] = {"u1", "u2"}
    fake.sets["bifrost:users:srv-b"] = {"u2", "u3"}
...
async def test_close_cleans_up_server_keys() -> None:
    ...
    assert f"bifrost:clients:{server.uuid}" in fake.deleted_keys
    assert ("bifrost:servers", server.uuid) in fake.srem_calls
...
async def test_listener_routes_event_to_channel_propagate(
    ...
    monkeypatch.setattr(
        "bifrost.server.server_channel.ServerChannel.propagate", fake_propagate
    )
