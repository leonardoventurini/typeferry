...
describe("mongodb registry integration", () => {
  ...
  it("returns native collections and metadata for registered tokens", async () => {
    ...
    try {
      ...
      expect(
        mongo.collectionByName<Board>(mongo.meta(BoardsCollection).name),
      ).toBe(Boards);
  ...
  it("creates declared indexes on demand and during startup", async () => {
    ...
    try {
      ...
      await expect(
        ...
        expect.arrayContaining([expect.objectContaining({ name: "author_1" })]),
      );
    } finally {
      await mongo.close();
    }
  ...
  it("enforces homogeneous variadic tuple schemas as non-empty arrays", async () => {
    @MongoCollection(harness.collectionName("variadic_tuple_events"))
    @MongoSchema(
      z.strictObject({
        changes: z.tuple([z.string()], z.string()),
      }),
    )
    class VariadicTupleEventsDefinition {}

    const EventsCollection = typedMongoCollection<{
      readonly changes: [string, ...string[]];
    }>(VariadicTupleEventsDefinition);
    const mongo = await createTypeFerryMongo({
      db: harness.db,
    ...
    try {
      const Events = mongo.collectionByName<Document>(
        mongo.meta(EventsCollection).name,
      );

      await expect(
        Events.insertOne({ changes: ["created", "reviewed"] }),
      ).resolves.toMatchObject({ acknowledged: true });
      await expect(Events.insertOne({ changes: [] })).rejects.toMatchObject({
        code: 121,
      });
      await expect(
        Events.insertOne({ changes: ["created", 1] }),
      ).rejects.toMatchObject({ code: 121 });
    } finally {
      await mongo.close();
    }
