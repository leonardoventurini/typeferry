...
export const MONGO_COLLECTION_DOCUMENT: unique symbol = Symbol(
  'typeferry.mongodb.collection.document',
)
...
/** Function that maps a changed document into one or more TypeFerry channels. */
...
export interface MongoWatchDefinition<TDocument extends Document = Document> {
  /** TypeFerry event name emitted for matching collection changes. */
  readonly event: string
  /** TypeFerry event options used when registering the event on the server. */
...
> {
  ...
  /** Change-stream definitions declared for TypeFerry event bridging. */
  readonly watches: readonly MongoWatchDefinition<TDocument>[]
}

/** Options accepted by `createTypeFerryMongo`. */
export interface TypeFerryMongoOptions {
  /** Optional TypeFerry server used for change-stream event bridging. */
...
/** Runtime returned by `createTypeFerryMongo`. */
export interface TypeFerryMongo {
...
/** Payload emitted from MongoDB change streams through TypeFerry events. */
