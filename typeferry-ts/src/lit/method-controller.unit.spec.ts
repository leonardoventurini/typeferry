...
import { TypeFerryEvents, NO_CHANNEL } from '../utils'
import EventEmitter2 from '../utils/event-emitter'
import { TypeFerryMethodController } from './method-controller'
...
describe('TypeFerryMethodController', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  })
  ...
  it('refreshes on mount and reacts to local and remote refresh events', async () => {
    ...
    const controller = new TypeFerryMethodController(host, client, {
      method: 'user.get',
      params: { id: 123 },
      channel: 'room-1',
      event: 'local-refresh',
    ...
    channel.emit(TypeFerryEvents.METHOD_REFRESH, 'user.get')
    await Promise.resolve()
    await Promise.resolve()
