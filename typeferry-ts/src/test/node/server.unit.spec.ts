...
describe('Server', () => {
  ...
  it('should close the server and remove the global instance', async () => {
    ...
    const originalTypeFerry = global.TypeFerry
    ...
    if (originalTypeFerry) {
      global.TypeFerry = originalTypeFerry
    }
  })
  ...
  it('should throw an error when trying to create a second instance', async () => {
    ...
    ).to.throw('There can only be one instance of TypeFerry.')
