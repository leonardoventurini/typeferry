...
import { TypeFerryEvents, Methods } from '../utils'
...
describe('ClientChannel', () => {
  ...
  describe('commitPendingSubscriptions', () => {
    it('emits {} (not null) when RPC_ON returns null', async () => {
      ...
      const emitSpy = vi.fn()
      channel.on(TypeFerryEvents.COMMIT_PENDING_SUBSCRIPTIONS, emitSpy)
  ...
  describe('commitPendingUnsubscriptions', () => {
    it('emits result when RPC_OFF succeeds', async () => {
      ...
      const emitSpy = vi.fn()
      channel.on(TypeFerryEvents.COMMIT_PENDING_UNSUBSCRIPTIONS, emitSpy)
