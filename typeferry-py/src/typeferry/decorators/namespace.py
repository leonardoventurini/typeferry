...
Mirrors ``typeferry-ts/src/server/decorators/namespace.ts``. Methods
registered from the decorated class are named ``<prefix>.<method_name>``
(or ``<prefix>.<wire_name>`` when :func:`~typeferry.decorators.method`
...
from typeferry.decorators.metadata import get_class_metadata
