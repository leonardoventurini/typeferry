...
export interface MongoLiveEngineDependencies {
  /** Bifrost server receiving live RPC registrations. */
...
export class MongoLiveEngine {
  ...
  private registerServerSurface(): void {
    ...
    for (const name of reserved) {
      if (server.methods.has(name)) {
        throw new Error(`Bifrost method "${name}" is already registered.`);
      }
    }
    if (server.events.has(MONGO_LIVE_EVENT)) {
      throw new Error(
        `Bifrost event "${MONGO_LIVE_EVENT}" is already registered.`,
      );
    }
  ...
  private deliver(
    ...
    const state = subscription.node.sendBifrostEvent(
      MONGO_LIVE_EVENT,
      NO_CHANNEL,
      delta,
    );
