 ...
 *
 * Spawns ``bifrost-rs/crates/bifrost-conformance-server`` via cargo,
 * parses the bound port from its stderr, then drives a Node-mode
 * Bifrost Client over the wire. Validates the Rust server is a
...
const RUST_DIR = path.join(REPO_ROOT, 'bifrost-rs')
...
const RUST_AVAILABLE =
  ...
  fs.existsSync(
    path.join(RUST_DIR, 'crates/bifrost-conformance-server/Cargo.toml'),
  ) &&
  cargoAvailable()
...
function readPortFromStderr(proc: ChildProcess): Promise<number> {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(
        new Error('timed out waiting for Rust server to print BIFROST_PORT'),
      )
    }, 90_000)
    let buffer = ''
    proc.stderr?.on('data', chunk => {
      buffer += chunk.toString('utf8')
      const match = buffer.match(/BIFROST_PORT=(\d+)/)
      if (match) {
        clearTimeout(timer)
...
describeIf('JS client ↔ Rust server (cross-language integration)', () => {
  ...
  beforeAll(async () => {
    ...
    const build = spawnSync(
      'cargo',
      ['build', '--quiet', '-p', 'bifrost-conformance-server'],
    ...
    proc = spawn(
      'cargo',
      ['run', '--quiet', '-p', 'bifrost-conformance-server'],
      {
        cwd: RUST_DIR,
