 ...
 *
 * Spawns ``bifrost-py/scripts/conformance_server.py`` as a subprocess,
 * parses the chosen port from its stderr, then drives a Node-mode
 * Bifrost Client over the wire. Validates the Python server is a
...
const PY_DIR = path.join(REPO_ROOT, 'bifrost-py')
...
/** Wait for `BIFROST_PORT=<n>` on stderr; resolve with the port. */
function readPortFromStderr(proc: ChildProcess): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    proc.stderr?.on('data', chunk => {
      ...
      const match = text.match(/BIFROST_PORT=(\d+)/)
      if (match) {
        clearTimeout(timer)
...
describeIf('JS client ↔ Python server (cross-language integration)', () => {
  ...
  beforeAll(async () => {
    // Force Node's `ws` package onto globalThis so the Bifrost Client
