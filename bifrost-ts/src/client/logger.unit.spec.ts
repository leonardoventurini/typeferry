...
import { BifrostLogger, LogCategory, LogLevel, logger } from './logger'

describe('BifrostLogger', () => {
  let log: BifrostLogger

  beforeEach(() => {
    log = new BifrostLogger()
  ...
  describe('output (console dispatch)', () => {
    ...
    it('includes context object when provided', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('[Bifrost:method]', 'ctx test', ctx)
    })
    ...
    it('includes error object when provided', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('[Bifrost:connection]', 'err test', err)
    })
    ...
    it('includes both context and error when both provided', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('[Bifrost:connection]', 'both', ctx, err)
    })
  })
  ...
  describe('category convenience methods', () => {
    ...
    it('auth() logs with AUTH category', () => {
      ...
      expect(console.log).toHaveBeenCalledWith('[Bifrost:auth]', 'auth msg')
    })
    ...
    it('subscription() logs with SUBSCRIPTION category', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('[Bifrost:subscription]', 'sub msg')
    })

    it('channel() logs with CHANNEL category', () => {
      log.channel(LogLevel.DEBUG, 'ch msg')
      expect(console.debug).toHaveBeenCalledWith('[Bifrost:channel]', 'ch msg')
    })
  })
  ...
  describe('shared logger instance', () => {
    it('exports a singleton logger', () => {
      expect(logger).toBeInstanceOf(BifrostLogger)
    })
  })
})
