...
describe('Server', () => {
  ...
  afterEach(async () => {
    ...
    delete (globalThis as any).Bifrost
  })
  ...
  describe('constructor', () => {
    ...
    it('adds METHOD_REFRESH event', () => {
      const s = createServer()
      expect(s.events.has('bifrost:method:refresh')).toBe(true)
    })
  })

  describe('globalInstance', () => {
    it('sets global Bifrost when globalInstance is true', () => {
      server = new Server({
        host: 'localhost',
        port: 0,
        globalInstance: true,
      ...
      expect((globalThis as any).Bifrost).toBe(server)
    })
    ...
    it('throws when creating second global instance', () => {
      ...
      ).toThrow('There can only be one instance of Bifrost.')
    })
  })
