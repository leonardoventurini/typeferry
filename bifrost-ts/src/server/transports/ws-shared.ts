...
export async function handleRpcVoid(
  ...
  if (!methodInstance) {
    console.warn('[Bifrost] Method not found for void call:', method)
    return
  }

  if (methodInstance.isProtected && !node.authenticated) {
    console.warn('[Bifrost] Method forbidden for void call:', method)
    return
  }
...
export async function authenticateNode(
  ...
  try {
    ...
    console.error('[Bifrost] Auth error:', error)
...
export function validateMeta(meta: unknown): Record<string, unknown> {
  ...
  try {
    if (JSON.stringify(meta).length > MAX_META_SIZE) {
      console.warn('[Bifrost] Meta object too large, ignoring')
      return {}
    }
  } catch {
    return {}
