...
//!
//! Exercises the frame-routing layer of `bifrost-ws` without needing a
//! real axum WebSocket. Covers PROTOCOL.md §5.1–5.2.

use async_trait::async_trait;
use bifrost_runtime::error::PublicError;
use bifrost_runtime::{
    BifrostError, BifrostSocket, ClientNode, MethodOptions, Server, ServerOptions,
    SocketState,
};
use bifrost_ws::dispatch_frame;
...
async fn rpc_frame_returns_result() {
    ...
    let dyn_socket: Arc<dyn BifrostSocket> = socket.clone();
...
async fn rpc_forbidden_when_unauthenticated() {
    ...
    let handler = Arc::new(|_n: Arc<ClientNode>, _p: Value| {
        async move { Ok(json!("secret")) }.boxed()
    });
    server.add_method(
        "s",
        handler,
...
async fn rpc_public_error_passes_message_through() {
    ...
    let handler = Arc::new(|_n: Arc<ClientNode>, _p: Value| {
        async move {
            Err(BifrostError::Public(PublicError::new("nope")))
        }
        .boxed()
    });
...
async fn rpc_void_sends_no_response() {
    ...
    let handler = Arc::new(|_n: Arc<ClientNode>, _p: Value| {
        async move {
            Err(BifrostError::Public(PublicError::new("silent")))
        }
        .boxed()
    });
    server.add_method("boom", handler, MethodOptions::default());
...
async fn rpc_schema_validation_surfaces_errors_field() {
    use bifrost_runtime::{SchemaValidator, ValidationIssue, ValidationResult};
