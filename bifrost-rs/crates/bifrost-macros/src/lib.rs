//! Authoring macros for the Bifrost Rust runtime — equivalent to the
//! TS / Python decorators.
//!
//! # Example
//!
//! ```ignore
//! use bifrost_macros::{method, register};
//! use bifrost_runtime::{ClientNode, BoxResult};
...
//! }
//!
//! fn wire(server: &Arc<bifrost_runtime::Server>) {
...
pub fn method(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let register_fn = Ident::new(
        &format!("__bifrost_register_{fn_name}"),
        Span::call_site(),
    );
    ...
    let expanded = quote! {
        ...
        pub fn #register_fn(
            server: &::std::sync::Arc<::bifrost_runtime::Server>,
            namespace: &str,
        ) {
            ...
            let handler: ::bifrost_runtime::RpcHandler = ::std::sync::Arc::new(
                |node, params| ::std::boxed::Box::pin(#fn_name(node, params))
            );
            let opts = ::bifrost_runtime::MethodOptions {
                protected: #protected,
                cache: #cached,
...
pub fn register(input: TokenStream) -> TokenStream {
    ...
    for ident in &methods {
        let register_fn = Ident::new(
            &format!("__bifrost_register_{ident}"),
            ident.span(),
        );
        calls.extend(quote_spanned! {ident.span()=>
            #register_fn(&__bifrost_server, __bifrost_namespace);
        });
    }

    let expanded = quote! {{
        let __bifrost_server: &::std::sync::Arc<::bifrost_runtime::Server> = &#server;
        let __bifrost_namespace: &str = #namespace_expr;
        #calls
    }};
