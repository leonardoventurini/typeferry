...
async fn public_error_passes_through() {
    ...
    let handler = Arc::new(|_n: Arc<ClientNode>, _p: Value| {
        async move {
            Err(bifrost_runtime::BifrostError::Public(PublicError::new(
                "intentional",
            )))
        }
        .boxed()
...
async fn internal_error_normalizes_unknown_exceptions() {
    ...
    let handler = Arc::new(|_n: Arc<ClientNode>, _p: Value| {
        async move {
            Err(bifrost_runtime::BifrostError::Internal(
...
async fn x_client_id_overrides_node_uuid() {
    let server = build_server();
    let handler = Arc::new(|node: Arc<ClientNode>, _p: Value| {
        async move { Ok(json!(node.uuid())) }.boxed()
    });
