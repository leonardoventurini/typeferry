//! Event primitive — port of `bifrost-py/src/bifrost/server/event.py`.
...
pub type ShouldSubscribe = Arc<
    dyn Fn(Arc<ClientNode>, String, String) -> ShouldSubscribeFuture + Send + Sync,
>;
...
impl Event {
    ...
    pub fn encode_payload(
        ...
        (serde_json::to_string(&payload).expect("event payload serializable"), exclude)
    }
}
