...
use bifrost_conformance::{configure_server, list_cases, load_json};
...
async fn http_fixtures_match_rust_runtime() {
    ...
    for case_path in cases {
        ...
        let router = bifrost_http::router(server);
        ...
        if let Some(expected_body) = fixture["response"].get("body").and_then(|v| v.as_str()) {
            ...
            let actual: Value = serde_json::from_str(&text).unwrap_or_else(|e| {
                panic!("{}: response was not JSON: {text} ({e})", case_path.display())
            });
            assert_eq!(
                &actual,
                expected_decoded,
