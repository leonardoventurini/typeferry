...
use axum::Router;
use bifrost_conformance::{configure_server, list_cases, load_ndjson};
use bifrost_runtime::{Server, ServerOptions};
...
async fn spawn_server(setup: Option<&Value>) -> (Arc<Server>, u16) {
    ...
    let app: Router = bifrost_ws::router(server.clone());
...
async fn next_text<S: StreamExt<Item = Result<Message, tokio_tungstenite::tungstenite::Error>> + Unpin>(
    stream: &mut S,
) -> String {
...
async fn ws_fixtures_match_rust_runtime() {
    ...
    for path in cases {
        ...
        assert_eq!(setup["op"], "setup", "{}: first op must be setup", path.display());
        let (server, port) = spawn_server(Some(&setup)).await;

        let connect = script.remove(0);
        assert_eq!(connect["op"], "connect", "{}: 2nd op must be connect", path.display());

        let url = format!(
            "ws://127.0.0.1:{port}/bifrost-ws{}",
        ...
        for op in script {
            ...
            match kind {
                ...
                "server_emit" => {
                    ...
                    let (payload, _exclude) =
                        event_obj.encode_payload(&channel, &params);
                    server.channel(&channel).propagate(&event, &payload, None).await;
                }
                "disconnect" => {
