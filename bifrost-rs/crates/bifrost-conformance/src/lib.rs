...
//! Rust runtime. Mirror of `bifrost-py/tests/conformance/harness.py`
//! and `bifrost-ts/src/test/conformance/harness.ts`.

use std::fs;
use std::path::{Path, PathBuf};
use std::sync::Arc;

use bifrost_ejson::{EjsonValue, InfNaNSign, Regex};
use bifrost_runtime::error::PublicError;
use bifrost_runtime::method::BoxResult;
use bifrost_runtime::{
    AuthFn, ClientNode, MethodOptions, RpcHandler, SchemaValidator, Server,
...
pub fn rehydrate(node: &Value) -> EjsonValue {
    ...
    match kind {
        ...
        "object" => {
            ...
            let mut out = bifrost_ejson::value::EjsonMap::new();
        ...
        "binary" => {
            ...
            let bytes = bifrost_ejson::base64::decode(b64).expect("valid base64");
...
pub fn build_handler(spec: &str) -> RpcHandler {
    ...
    if let Some(message) = spec.strip_prefix("raise_public:") {
        ...
        return Arc::new(move |_n, _p| {
            let msg = owned.clone();
            async move {
                Err(bifrost_runtime::BifrostError::Public(PublicError::new(msg)))
            }
            .boxed()
        });
    }
...
pub fn build_schema(spec: Option<&Value>) -> Option<Arc<dyn SchemaValidator>> {
    let spec = spec?;
    if !spec.get("reject_all").and_then(|v| v.as_bool()).unwrap_or(false) {
        return None;
    }
...
pub fn configure_server(server: &Arc<Server>, setup: Option<&Value>) {
    ...
    if let Some(events) = setup.get("events").and_then(|v| v.as_array()) {
        for spec in events {
            let name = spec["name"].as_str().unwrap().to_string();
            server.add_event(bifrost_runtime::Event::new(
                &name,
                bifrost_runtime::EventOptions::default(),
            ));
        }
    }
