...
use typeferry_runtime::{RoomRegistry, SocketState, TypeFerrySocket};
...
fn join_and_has() {
    ...
    rooms.join(s.clone() as Arc<dyn TypeFerrySocket>, "room-a");
...
fn leave_all_removes_from_every_room() {
    let rooms = RoomRegistry::new();
    let s = MockSocket::new(1);
    rooms.join(s.clone() as Arc<dyn TypeFerrySocket>, "a");
    rooms.join(s.clone() as Arc<dyn TypeFerrySocket>, "b");
    rooms.leave_all(&*s);
...
async fn broadcast_sends_to_every_open_socket() {
    ...
    closed.mark_closed();
    rooms.join(a.clone() as Arc<dyn TypeFerrySocket>, "room");
    rooms.join(b.clone() as Arc<dyn TypeFerrySocket>, "room");
    rooms.join(closed.clone() as Arc<dyn TypeFerrySocket>, "room");
