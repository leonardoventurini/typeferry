...
fn http_router_is_reachable() {
    let server = Server::new(ServerOptions::default());
    let _ = typeferry::http::router(server);
}
...
fn ws_router_is_reachable() {
    let server = Server::new(ServerOptions::default());
    let _ = typeferry::ws::router(server);
}

#[cfg(feature = "auth")]
#[test]
fn auth_namespace_is_reachable() {
    let _ = typeferry::auth::AuthConfig::new("unit-test-secret-that-is-sufficiently-long");
}
...
mod macros_smoke {
    ...
    async fn macro_register_works_via_umbrella() {
        let server = Server::new(ServerOptions::default());
        typeferry::register!(server, [ping]);
        let result = server
