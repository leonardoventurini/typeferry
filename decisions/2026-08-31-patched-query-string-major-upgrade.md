...
Bifrost uses `query-string` solely in `Client.href()` to serialize a final
...
The dependency upgrade crosses two major versions, but Bifrost already targets
...
Upgrade Bifrost's production dependency from `query-string@^7.1.3` to
`query-string@^9.5.0` and publish the result as Bifrost `0.4.2`.
...
the compatibility regression protects Bifrost's observable behavior across
the major-version jump.

## Consequences

- Bifrost consumers receive `decode-uri-component@0.5.0` through the supported
  dependency graph.
- The Bifrost dependency tree audits clean after frozen installation.
- Consumers must upgrade to Bifrost `0.4.2` or later to receive the fix.
