# TypeFerry project template architecture

## Context

The TypeFerry repository needed a small but production-minded application starter
...
- Use protected TypeFerry methods and owner-scoped events. Events invalidate
...
- Bundle the Node server and transitive runtime dependencies into CommonJS for
  ...
  CommonJS-only transitive package currently consumed by the published TypeFerry
...
- Build a multi-stage production image and run only the built output as the
  non-root Node user. The TypeFerry-owned Hono app serves Vite assets, SPA
  fallback, health, RPC, and WebSocket traffic from one container and port.
- Use a separate development image for the existing Vite/TypeFerry orchestrator.
...
- Running the published TypeFerry ESM output directly was rejected after the real
  startup smoke exposed a named-export incompatibility in a CommonJS transitive
  dependency. Bundling resolves that boundary without patching TypeFerry or its
  installed output.

## Rationale

These choices leave one obvious path for each concern: Mise for tools, npm for
packages, MongoDB driver for persistence, TypeFerry for typed transport/events,
...
- Path-sensitive tools must enumerate the root-level application directories;
  ...
  updates.
- Updating Node/npm, Tailwind, MongoDB, or TypeFerry requires coordinated manifest,
