...
describe('Events', () => {
  ...
  it('should exclude originator when excludeOriginator is true', async () => {
    const clientB = await test.createClient()

    test.server.addEvent('exclude:event', { excludeOriginator: true })

    await test.client.subscribe('exclude:event')
    await clientB.subscribe('exclude:event')

    // Get client A's node to get its UUID
    const clientNodeA = test.server.allClients.get(test.client.uuid)!

    test.server.defer('exclude:event', { uuid: clientNodeA.uuid, data: 'test' })

    const clientATimeout = await test.client.timeout('exclude:event')
    expect(clientATimeout).to.be.true

    test.server.defer('exclude:event', { uuid: clientNodeA.uuid, data: 'test' })

    const clientBResult = await clientB.wait('exclude:event')
    expect(clientBResult).to.have.property('data', 'test')

    await clientB.close()
  })

  it('should send to all clients when excludeOriginator uuid is not in params', async () => {
    const clientB = await test.createClient()

    test.server.addEvent('exclude:event2', { excludeOriginator: true })

    await test.client.subscribe('exclude:event2')
    await clientB.subscribe('exclude:event2')

    // Both clients should receive the event when no uuid is provided
    const resultAPromise = test.client.wait('exclude:event2')
    const resultBPromise = clientB.wait('exclude:event2')

    test.server.defer('exclude:event2', { data: 'no-uuid' })

    const [resultA, resultB] = await Promise.all([
      resultAPromise,
      resultBPromise,
    ])
    expect(resultA).to.have.property('data', 'no-uuid')
    expect(resultB).to.have.property('data', 'no-uuid')
