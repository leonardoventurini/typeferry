...
import type { BifrostContext } from './context-manager'
import { ContextManager } from './context-manager'
import { IdleTimer } from './idle-timer'
import type { BifrostLogger } from './logger'
...
> extends ClientChannel {
  ...
  get context(): BifrostContext {
    return this.contextManager.context
  }

  set context(value: BifrostContext) {
  ...
  async close() {
    ...
    this.channels.forEach(channel => {
      channel.emit(BifrostEvents.COMMIT_PENDING_SUBSCRIPTIONS, {})
    })

    this.channels.forEach(channel => {
      channel.emit(BifrostEvents.COMMIT_PENDING_UNSUBSCRIPTIONS, {})
    })
  ...
  private async waitForInitialization(timeout: number) {
    try {
      ...
      throw new Error('Bifrost: Client not initialized')
    }
  }
