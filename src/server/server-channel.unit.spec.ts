...
import { BifrostEvents } from '../utils'
...
describe('ServerChannel', () => {
  ...
  describe('propagate', () => {
    ...
    it('warns and returns when webSocketTransport is null', () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        '[Bifrost] WebSocket transport not available for propagate',
      )
      warnSpy.mockRestore()
    })
  ...
  describe('refresh', () => {
    it('emits BifrostEvents.METHOD_REFRESH with method name', () => {
      // Register the METHOD_REFRESH event to avoid warning
      server.events.set(
        BifrostEvents.METHOD_REFRESH,
        createMockEvent(BifrostEvents.METHOD_REFRESH),
      )
