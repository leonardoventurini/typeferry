...
import {
  ...
  type BifrostMethodControllerOptions,
...
export class BifrostMethodController extends BifrostClientBoundController {
  ...
  setOptions(options: BifrostMethodControllerOptions): void {
  ...
  private rebindSubscriptions(nextBindKey?: RebindKey): void {
    ...
    this.addCleanup(() => {
      channel.off(BifrostEvents.METHOD_REFRESH, this.handleRemoteMethodRefresh)
      this.clearRemoteUnsubscribeTimer()
      this.remoteUnsubscribeTimer = setTimeout(() => {
        const listeners = channel._events?.[BifrostEvents.METHOD_REFRESH]
        const hasListeners = Array.isArray(listeners)
          ? listeners.length > 0
          : !!listeners

        if (!hasListeners) {
          channel.unsubscribe(BifrostEvents.METHOD_REFRESH).catch(console.error)
        }
      }, REMOTE_UNSUBSCRIBE_DELAY_MS)
    })

    channel.subscribe(BifrostEvents.METHOD_REFRESH).catch(console.error)
  }
