...
import { BifrostSubscribeController } from './subscribe-controller'
...
describe('Lit controllers', () => {
  ...
  it('tracks auth and connection state', async () => {
    ...
    const auth = new BifrostAuthController(host, client)
    const connection = new BifrostConnectionController(host, client)
  ...
  it('wires local event handlers to the requested channel', () => {
    ...
    const controller = new BifrostLocalEventController(host, client, {
      event: 'ping',
      channel: 'room-1',
      callback,
    })
  ...
  it('subscribes to remote events and unsubscribes on disconnect', async () => {
    ...
    const controller = new BifrostSubscribeController(host, client, {
      event: 'message',
      channel: 'room-2',
      callback,
    })
