...
describe('EJSON clone functionality', () => {
  ...
  test('clones deeply nested arrays with objects', () => {
    const nested = {
      items: [
    ...
    performCloneTest(nested)
  })

  test('clones arrays containing multiple levels of nesting', () => {
    const complex = [{ a: [{ b: [{ c: 1 }] }] }, { d: [{ e: [{ f: 2 }] }] }]
    performCloneTest(complex)
  })

  test('clones todo-like structures without stack overflow', () => {
    // This structure previously caused stack overflow when cloneArray
    // called clone() instead of internalClone()
    const todos = {
      boardId: 'board-123',
      nodeId: 'node-456',
      todos: [
        { id: 'a', text: 'Task 1', completed: false, order: 0 },
        { id: 'b', text: 'Task 2', completed: true, order: 1 },
        { id: 'c', text: 'Task 3', completed: false, order: 2 },
      ],
    }
    performCloneTest(todos)
  })

  test('clones large arrays efficiently', () => {
    const largeArray = Array.from({ length: 100 }, (_, i) => ({
      id: `item-${i}`,
      nested: { value: i, tags: ['a', 'b', 'c'] },
    }))
    performCloneTest(largeArray)
  })
})
